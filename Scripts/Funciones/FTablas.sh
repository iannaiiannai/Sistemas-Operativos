			...
			case $tipo in
				...
				"date") 
						while true; do
							...
							if [ "${fecha[0]}" -gt 999 ] && [ "${fecha[0]}" =~ ^-?[0-9]+$ ]; then
								
								while true; do
									read -p "Ingrese un mes: " fecha[1]
									if [ "${fecha[1]}" -lt 13 ] && [ "${fecha[1]}" -gt 0 ] && [ "${fecha[1]}" =~ ^-?[0-9]+$ ]; then
										if [ "${fecha[1]}" -lt 10 ] && [ "${fecha[1]}" -gt 0 ] && [ "${fecha[1]}" =~ ^-?[0-9]+$ ]; then
											fecha[1]="0""${fecha[1]}"

											while true; do
												read -p "Ingrese un día: " fecha[2]
												if [ $(($bisiesto % 4)) -eq 0 ] && { [ $(($bisiesto % 100)) -ne 0 ] || [ $(($bisiesto % 400)) -eq 0 ]; }
													if [ "${fecha[1]}" == "02" ]; then
														if [ "${fecha[2]}" -gt 0 ] && [ "${fecha[2]}" -lt 30] && [ "${fecha[2]}" =~ ^-?[0-9]+$ ]; then
															break
														else
														...
														if [ "${fecha[2]}" -gt 0 ] && [ "${fecha[2]}" -lt 32] && [ "${fecha[2]}" =~ ^-?[0-9]+$ ]; then
															break
														else
															echo "Ingrese una fecha válida"
															echo " "
														...
														if [ "${fecha[2]}" -gt 0 ] && [ "${fecha[2]}" -lt 31] && [ "${fecha[2]}" =~ ^-?[0-9]+$ ]; then
															break
														else
															echo "Ingrese una fecha válida"
															echo " "
													...
													if [ "${fecha[1]}" == "02" ]; then
														if [ "${fecha[2]}" -gt 0 ] && [ "${fecha[2]}" -lt 29] && [ "${fecha[2]}" =~ ^-?[0-9]+$ ]; then
															break
														else
															echo "Ingrese una fecha válida"
															echo " "
